...
title: '渣打倍多纷白金信用卡'
shortName: '倍多纷白金'
slug: standard-chartered-platinum-cashback-credit-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: scPlatinumCashBackCreditCard
...
> 渣打倍多纷白金信用卡按现金回赠、基本奖赏与 FTF 规则收录。
...
| 基本回赠 | 1% |
| 奖赏单位 | HKD |
| 计算器插件 | `scPlatinumCashBackCreditCard` |

## 概述

**渣打倍多纷白金信用卡** 是 [[banks:standard-chartered-hong-kong|渣打香港]](../banks/standard-chartered-hong-kong.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `scPlatinumCashBackCreditCard` |
| 兑换计算器 | 否 | `—` |
