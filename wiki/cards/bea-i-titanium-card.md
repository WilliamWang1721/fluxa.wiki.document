---
collection: cards
title: '东亚银行 i-Titanium 卡'
shortName: 'i-Titanium'
slug: bea-i-titanium-card
country: HK
...
> 东亚银行 i-Titanium 卡按现金回赠 / 基本奖赏与 FTF 规则收录。

| 字段 | 值 |
| --- | --- |
| 官方名称 | 东亚银行 i-Titanium 卡 |
| 简称 | i-Titanium |
| 市场 | 香港 |
...
| 基本回赠 | 0.4% |
| 奖赏单位 | HKD |
| 计算器插件 | `beaITitaniumCard` |

## 概述

**东亚银行 i-Titanium 卡** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| --- | --- |
| 基本现金回赠 | 0.4% |
| 网上购物 / 移动支付额外 i-Dollar | 额外 3.2% |

## 计算器接入

| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `beaITitaniumCard` |
| 兑换计算器 | 否 | `—` |
