...
> eye 信用卡网上及本地消费按基本奖赏与指定类别官方口径计算。
...
| 银联双币 | 否 |
| 计算器插件 | `ccbAsiaEyeCreditCard` |

## 概述

**建行亚洲 eye 信用卡** 是 [[banks:ccb-asia|建行亚洲]](../banks/ccb-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
