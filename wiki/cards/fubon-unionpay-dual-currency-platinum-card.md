...
title: '富邦银行银联双币白金卡'
shortName: '银联双币白金'
slug: fubon-unionpay-dual-currency-platinum-card
country: HK
bank: fubon
...
rebateCalculator:
  enabled: true
  calculatorId: fubonUnionPayDualCurrencyPlatinum
...
> 富邦银行银联双币白金卡按银联 / 内地签账和 FTF 官方条款收录。
...
| 奖赏单位 | Bonus Points |
| 计算器插件 | `fubonUnionPayDualCurrencyPlatinum` |

## 概述

**富邦银行银联双币白金卡** 是 [[banks:fubon|富邦银行]](../banks/fubon.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `fubonUnionPayDualCurrencyPlatinum` |
| 兑换计算器 | 否 | `—` |
