---
collection: cards
title: '东亚银行澳洲会计师公会信用卡'
shortName: 'CPA Australia'
slug: bea-cpa-australia-credit-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: beaCpaAustraliaCreditCard
...
| 奖赏单位 | HKD |
| 计算器插件 | `beaCpaAustraliaCreditCard` |

## 概述

**东亚银行澳洲会计师公会信用卡** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `beaCpaAustraliaCreditCard` |
| 兑换计算器 | 否 | `—` |
