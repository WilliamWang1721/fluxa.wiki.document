---
collection: cards
title: '东亚银行职业训练局 Visa 卡（学生专用）'
shortName: 'VTC 学生 Visa'
slug: bea-vtc-student-visa-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: beaVtcStudentVisaCard
...
# 东亚银行职业训练局 Visa 卡（学生专用）

> 东亚银行职业训练局 Visa 学生卡按教育 / 基本奖赏与 FTF 规则收录。

| 字段 | 值 |
| --- | --- |
| 官方名称 | 东亚银行职业训练局 Visa 卡（学生专用） |
| 简称 | VTC 学生 Visa |
| 市场 | 香港 |
...
| 奖赏单位 | HKD |
| 计算器插件 | `beaVtcStudentVisaCard` |

## 概述

**东亚银行职业训练局 Visa 卡（学生专用）** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
