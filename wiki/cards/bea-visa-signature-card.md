---
collection: cards
title: '东亚银行 Visa Signature 卡'
shortName: 'Visa Signature'
slug: bea-visa-signature-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: beaVisaSignatureCard
...
# 东亚银行 Visa Signature 卡

> 东亚银行 Visa Signature 卡按官方积分、海外签账及 FTF 条款配置。

| 字段 | 值 |
| --- | --- |
| 官方名称 | 东亚银行 Visa Signature 卡 |
| 简称 | Visa Signature |
| 市场 | 香港 |
...
| 奖赏单位 | HKD |
| 计算器插件 | `beaVisaSignatureCard` |

## 概述

**东亚银行 Visa Signature 卡** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 本地餐饮 6X | 额外 5 points / HK$1 = 2% |
| 外币签账 4X | 额外 3 points / HK$1 = 1.2% |

## 计算器接入

| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `beaVisaSignatureCard` |
| 兑换计算器 | 否 | `—` |
