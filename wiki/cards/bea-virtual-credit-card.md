...
rebateCalculator:
  enabled: true
  calculatorId: beaVirtualCreditCard
...
| 奖赏单位 | HKD |
| 计算器插件 | `beaVirtualCreditCard` |

## 概述

**东亚银行虚拟信用卡** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
