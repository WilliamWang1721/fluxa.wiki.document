...
| 银联双币 | 是 |
| 计算器插件 | `ccbAsiaIndustryUnionPayDualCurrency` |

## 概述

**建行亚洲建造业银联双币信用卡** 是 [[banks:ccb-asia|建行亚洲]](../banks/ccb-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `ccbAsiaIndustryUnionPayDualCurrency` |
| 兑换计算器 | 否 | `—` |
