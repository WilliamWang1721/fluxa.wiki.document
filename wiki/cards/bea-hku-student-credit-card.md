---
collection: cards
title: '东亚银行香港大学信用卡（学生专用）'
shortName: '港大学生'
slug: bea-hku-student-credit-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: beaHkuStudentCreditCard
...
> 东亚银行香港大学学生信用卡按教育 / 基本奖赏与 FTF 规则收录。

| 字段 | 值 |
| --- | --- |
| 官方名称 | 东亚银行香港大学信用卡（学生专用） |
| 简称 | 港大学生 |
| 市场 | 香港 |
...
| 奖赏单位 | HKD |
| 计算器插件 | `beaHkuStudentCreditCard` |

## 概述

**东亚银行香港大学信用卡（学生专用）** 是 [[banks:bank-of-east-asia|东亚银行]](../banks/bank-of-east-asia.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `beaHkuStudentCreditCard` |
| 兑换计算器 | 否 | `—` |
