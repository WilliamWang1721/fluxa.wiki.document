...
title: '汇丰现金回馈御玺卡'
shortName: '现金回馈御玺'
slug: hsbc-tw-cashback-signature-credit-card
country: TW
bank: hsbc-taiwan
...
rebateCalculator:
  enabled: true
  calculatorId: globalHsbcTwCashbackSignatureCreditCard
  issuer: 'HSBC TW'
...
> 台湾 HSBC 官方信用卡产品。官方名称：Cashback Signature Credit Card。状态：官方产品页列出，未见停售标记。
...
| 银联双币 | 否 |
| 计算器插件 | `globalHsbcTwCashbackSignatureCreditCard` |
| 英文官方名 | Cashback Signature Credit Card |
| 产品状态 | 官方产品页列出，未见停售标记 |

## 概述

**汇丰现金回馈御玺卡** 是 [[banks:hsbc-taiwan|汇丰台湾]](../banks/hsbc-taiwan.md) 在台湾市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `globalHsbcTwCashbackSignatureCreditCard` |
| 兑换计算器 | 否 | `—` |
