...
rebateCalculator:
  enabled: true
  calculatorId: fubonVisaPlatinumCard
...
> 富邦银行 Visa 白金卡按官方 Bonus Points、外币 / 周末奖赏及 FTF 条款配置。
...
| 基本回赠 | 1 point / HK$1 |
| 奖赏单位 | Bonus Points |
| 计算器插件 | `fubonVisaPlatinumCard` |

## 概述

**富邦银行 Visa 白金卡** 是 [[banks:fubon|富邦银行]](../banks/fubon.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
