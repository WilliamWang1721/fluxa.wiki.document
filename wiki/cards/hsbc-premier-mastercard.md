...
> 基础 RewardCash、Premier Rewards of Your Choice 与 FTF 计算。

| 字段 | 值 |
| --- | --- |
| 官方名称 | 汇丰卓越理财 Mastercard |
...
| 银联双币 | 否 |
| 计算器插件 | `hsbcPremierMasterCard` |
| 兑换规则 | `premierMasterCard` |

## 概述

**汇丰卓越理财 Mastercard** 是 [[banks:hsbc-hong-kong|汇丰香港]](../banks/hsbc-hong-kong.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
...
| 类型 | 是否接入 | 标识 |
| --- | --- | --- |
| 返利计算器 | 是 | `hsbcPremierMasterCard` |
| 兑换计算器 | 是 | `premierMasterCard` |
