---
collection: cards
title: 'MANHATTAN id 信用卡'
shortName: 'MANHATTAN id'
slug: manhattan-id-credit-card
country: HK
...
rebateCalculator:
  enabled: true
  calculatorId: manhattanIdCreditCard
...
> MANHATTAN id 信用卡按娱乐 / 生活类别、基本奖赏与 FTF 规则收录。

| 字段 | 值 |
| --- | --- |
| 官方名称 | MANHATTAN id 信用卡 |
| 简称 | MANHATTAN id |
| 市场 | 香港 |
...
| 计算器插件 | `manhattanIdCreditCard` |

## 概述

**MANHATTAN id 信用卡** 是 [[banks:standard-chartered-hong-kong|渣打香港]](../banks/standard-chartered-hong-kong.md) 在香港市场发行的信用卡词条。本页是 Fluxa WikiCard 的 Git 暂存稿：主站 [fluxa.wiki](https://fluxa.wiki) 的正文目前写在 Payload 数据库里，等 `payload-website-starter` 代码稳定后再迁回 Cards collection。
